...
function loadPosts(){
  ...
  config.postagensConhecidas.forEach(post => {
    if(switter[post].id == personagemId){
      let data = new Date(Number(switter[post].data));
      ...
      document.getElementsByClassName("feed")[0].appendChild(create("div", {className:"post", appendChild:[
        ...
        create("div", {className:"post-content", innerHTML:switter[post].conteudo})
      ]}));
    }
  });
}
